...
def show_output_impropers_file_content(impropers_output_file_paths):
    ...
    # Check if 'impropers.ndx' is present in the output file list
    ...
    if impropers_ndx_path:
        ...
        st.error("'impropers.ndx' not found in the output files")
        return
...
def func_page_replicate():
    ...
    with st.expander("OPTIONS"):
        ...
        if option != "Select impropers file (NDX)":
            ...
            if need_impropers:
                option_get_impropers = st.session_state.get("option_get_impropers", {})
                if not option_get_impropers:
                    option_get_impropers = {}

                input_sh_option = [
                    "Select a script to generate an impropers file (SH)*"
                ]

                for index_get_improper, option_sh in enumerate(input_sh_option, start=1):
                    input_sh_key = f"sh_file_{index_get_improper}"

                    if input_sh_key not in option_get_impropers:
                        option_get_impropers[input_sh_key] = ""
                    ...
                    with col1:
                        ...
                        if st.button("Browse file", key=browse_button_sh_key):
                            ...
                            if option_sh == "Select a script to generate an impropers file (SH)*":
                            ...
                            if sh_input_filename:
                                option_get_impropers[input_sh_key] = sh_input_filename

                    with col2:
                        if option_get_impropers[input_sh_key]:
                            remove_button_sh_key = f"remove_{input_sh_key}"
                            if st.button("Remove file", key=remove_button_sh_key):
                                option_get_impropers[input_sh_key] = ""

                st.session_state["option_get_impropers"] = option_get_impropers

                compressed_improper_file_name = st.text_input("Enter a name for the output compressed impropers file")

                # Button to execute the script
                if st.button("Run '.sh' file and generate IMPROPERS File"):
                    input_sh_file_path = option_get_impropers.get("sh_file_1", "")
                    ...
                    if not compressed_improper_file_name:
                        st.error("Please enter a name for the output compressed impropers file")
                        return
                    ...
                    if input_sh_file_path is not None:
                        with st.spinner("Running script. Please wait..."):
                            with tempfile.TemporaryDirectory() as improper_temp_dir:
                                ...
                                if compressed_improper_file_name:
                                    ...
                                    download_link = (f'<a href="data:application/tar+gzip;base64,'
                                                     ...
                                                     f'" download="{compressed_improper_file_name}_file.tar.gz">'
                                                     f'Download {compressed_improper_file_name} output file</a>')
