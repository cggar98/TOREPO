...
class StmolScreen:
    ...
    def show_screen(self):
        ...
        self._residues_selected_color = st.sidebar.color_picker(label="Residues selected color", value="#00FF00")
        ...
        if file_to_see is not None:
            ...
            xyzview.addSurface(py3Dmol.VDW, {"opacity": self._surf_transp, "color": self._backbone_color},
                               {"hetflag": False})  # Surface transparence does not work
            ...
            #   TEST
            #####   DOES NOT IDETIFIQUI RESIDUE
            # Si se especifica el nombre del residuo pero no el número de cadena ni el número de átomo
            if self._residue_name and not self._chain_number and not self._atom_number:
                xyzview.addStyle({"resn": self._residue_name},
                                 {"stick": {"color": self._residues_selected_color, "radius": stick_radius}})

                xyzview.addResLabels({"resn": self._residue_name},
            ...
            if not self._residue_name and not self._chain_number:
                for hl_atom in self._atom_number:
                    # p.setStyle({'serial': h}, {'stick': {'color': 'purple'}});
