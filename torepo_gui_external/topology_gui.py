...
class TopologyScreen:
    ...
    def show_screen(self):
        ...
        with left_col:
            ...
            for index, option in enumerate(self._input_files):
                ...
                if self._input_options[input_key]:
                    file_name = os.path.basename(self._input_options[input_key])
                    st.markdown(f"<span style='color:yellow;'>*{file_name}*</span>",
        ...
        with right_col:
            ...
            self._pattern = st.text_input("**String pattern to name the new files***",
                                          st.session_state.get("pattern", ""))
            ...
            self._isunwrap = st.toggle("Unwrap coordinates in the final structure",
                                       st.session_state.get("isunwrap", False))
        ...
        if button_run:
            if not self._server_valid:
                st.error("ERROR!!! Please enter all server options correctly")
                return
            ...
            if not input_file_path:
                st.error("ERROR!!! Please select an input file (XSD, PDB or MOL2) before running the program")
                return

            st.session_state["input_options"] = self._input_options

            if not self._pattern:
                st.error("ERROR!!! Please string pattern to name the new files")
                return
