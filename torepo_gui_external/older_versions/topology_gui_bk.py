...
def func_page_topology():
    ...
    with st.expander("OPTIONS"):
        ...
        # Verifica el contenido del archivo PDB


        # stmol options
        visor_file = st.file_uploader("Enter a file to see the molecule")
        button_visual = st.button("VER")
        if button_visual:
            if visor_file:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".pdb") as tmp_file:
                    tmp_file.write(visor_file.getbuffer())
